...
- [ ] **A3** shiftType 하드코딩 → 커스텀 파트 시 공평성 붕괴 — F 공평 지표 재설계로 진행
...
- [x] **A8** 고용형태(parttime) 미반영 — 제품 범위 제외, 풀타임/파트타임 개념 없음
...
**재계획 상태**  
`plan/fairness-calc-review.md` 검토 결과, 단순히 `shift_parts`에 수동 부담 컬럼을 추가하는 계획은 폐기한다.  
새 방향은 **파트 부담을 입력받지 않고 `startTime`/`endTime`에서 자동 도출**하는 것이다. 상세 실행 계획은 섹션 F로 이동한다.
...
**수정 방향**  
- Phase 1: 스키마 변경 없이 시간 기반 `partBurden(part)`를 도입해 커스텀 파트 부담을 자동 계산
- Phase 2: 파트별 성향 JSON을 도입해 `open/middle/close_preference` 하드코딩 제거
- Phase 3: 근무 요일 부담과 최근성 윈도우를 반영해 오래된 로그가 점수를 영구 지배하지 않게 개선
...
### A8 — 고용형태(parttime) 미반영 `[처리 완료: 범위 제외]` `[시연 영향: 낮음]`

**처리 상태**  
완료. 현재 제품에는 풀타임/파트타임 개념을 두지 않는다. 생성 룰은 모든 활성 직원을 동일한 근무 가능 모델로 취급한다.  
`employmentType` 필드는 현 DB 호환을 위해 남겨 두되, 스케줄러 개선 범위에서는 무시한다.
...
**수정 방향**  
- 추후 스키마 정리 라운드에서 `employmentType` 필드 제거 여부를 검토한다.
...
- 풀타임/파트타임 개념은 제품 범위에서 제외하므로 A8은 무시한다.
