...
> 참고: `availableDays` 필드는 스키마에 남아 있지만, 최근 제품 방향에서는 직원관리 화면에서 근무 가능 요일 UI를 제거했다. 따라서 A1은 코드상 결함이 맞지만, 즉시 복구할 기능인지 아니면 레거시 필드를 정리할지 제품 결정을 먼저 해야 한다.
...
- [ ] **A1** 근무 가능 요일 필드와 현재 제품 방향 불일치 — 결정 필요
- [ ] **A2** 파트당 최소 인원 0명 가능
...
- [ ] **C1** [week] 사후 수정·확정·되돌리기 UI 부재
- [ ] **C2** 수정 시 공평 점수 실시간 재계산 없음
- [ ] **C3** 모바일 캘린더 깨짐 + 비반응형 네비
- [ ] **C4** 출력/공유 기능 전무
...
### A1 — 근무 가능 요일 필드와 현재 제품 방향 불일치 `[결정 필요]` `[시연 영향: 낮음~높음]`
...
단, 현재 직원관리 UI에서는 근무 가능 요일 입력을 제거한 상태다. 모든 직원을 전 요일 근무 가능으로 보는 것이 현재 제품 결정이라면, 이 항목은 치명 결함이 아니라 레거시 스키마/PRD 불일치다.

**수정 방향**  
- 근무 가능 요일 기능을 복구한다면: `generateWeekSchedule`에서 `availableDays`를 하드 제약으로 적용하고, 출근 불가일은 주 2회 휴무 소진과 별도로 처리
- 복구하지 않는다면: `availableDays` 필드와 PRD 목업 설명을 정리해 "전 요일 근무 가능" 모델로 명확화
...
- `/schedule/[week]/view` 공유 전용 읽기전용 뷰 구현  
...
| 캘린더 비시맨틱 div | `schedule-calendar.tsx:43` | `<table>` + `<th scope="col">` 구조로 전환 |
...
| **3순위** | C1 [week] 수정·확정 워크플로우, C4 공유 | PRD 핵심 가치, 시연 필수 |
| **4순위** | A2 최소 인원, A4 클로프닝, A5 동점 처리, A7 타임존 | 생성 품질과 신뢰도 |
| **5순위** | A1 availableDays 제품 결정, A3 커스텀파트 공평성, C2 실시간 공평 점수 | 제품 정책 확정 후 반영할 영역 |
| **후속** | A6, A8, A9, B5~B6, C3, C6~C7 | 점진적 개선 |
