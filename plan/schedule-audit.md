...
- [x] 근무 부담에도 요일 계수 적용: `weekday=1`, `weekend=1.5`, `holiday=2`
- [x] 최근 N주 윈도우 옵션 추가, 기본 8주부터 검토
- [x] `/api/fairness`와 생성 API의 과거 로그 조회 범위를 동일 기준으로 맞춤
- [ ] 실제 데이터 또는 목업 분포를 보고 상수 보정 필요 여부 결정

### F4 — 동점 처리 정리

- [x] 생성 경로에는 seed 기반 동점 셔플 적용 완료
- [x] 공평 지표 표시용 정렬에는 `점수 desc → 이름 → id` 보조 키 적용
- [ ] 누적 휴무 보상 breakdown이 필요해지면 `점수 desc → 누적 휴무 보상 asc → 이름` 재검토
